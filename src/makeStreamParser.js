        ...
        .then(function (rootData) {
          ...
          Promise.all(rootData.subResources.map(path => resolveOne(zip, path)))
            .then(function (filesData) {
              ...
              rootData.build = rootData.build.map(replaceReference).filter(x => x !== undefined)
              // this is for
              Object.keys(rootData.objects).forEach(function (key) {
                const object = rootData.objects[key]
                object.components = object.components.map(replaceReference).filter(x => x !== undefined)
              })
