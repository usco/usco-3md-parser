    ...
    new JSZip().loadAsync(data).then(function (zip) {
      resolveOne(zip, '3D/3dmodel.model', true).then(function (rootData) {
        ...
        Promise.all(rootData.subResources.map(function (path) {
          ...
          function replaceReference(item) {
            ...
            var objectOffsetId = void 0;
            var key = path + id;
            if (byPath[key]) {
              objectOffsetId = byPath[key].id;
            } else {
              var resolved = resolvedPath.objects[id];
              byPath[key] = { id: offsetIndex, data: resolved };
              objectOffsetId = offsetIndex;
              //simpleObjects[offsetIndex] = resolved
              rootData.objects[offsetIndex] = resolved;
              offsetIndex += 1;
            }

            item = Object.assign({}, item, { objectid: objectOffsetId });
            //console.log(item)
            return item;
          }

          rootData.build = rootData.build.map(replaceReference).filter(function (x) {
            return x !== undefined;
          });
          // this is for
          Object.keys(rootData.objects).forEach(function (key) {
            var object = rootData.objects[key];
            object.components = object.components.map(replaceReference).filter(function (x) {
              return x !== undefined;
            });
          });

          //rootData.simpleObjects = simpleObjects
